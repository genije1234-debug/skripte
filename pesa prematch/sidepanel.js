...
const STAKE_BALANCE_OFFSET_TZS = 100;
...
function updateBalanceDisplay(balance, timestamp) {
  if (liveBalanceValueEl) {
    liveBalanceValueEl.textContent = `${formatAmount(balance)} TZS`;
  }
...
function applyStakeFromBalance(balance, timestamp) {
  ...
  const nextStake = Number((normalizedBalance + STAKE_BALANCE_OFFSET_TZS).toFixed(2));
  if (stakeInputEl) {
...
document.getElementById('golBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    // Find SportPesa tab
    let tabs = await chrome.tabs.query({ url: '*://sportpesa.co.tz/*' });
    if (!tabs || tabs.length === 0) {
...
document.getElementById('captureCookiesBtn').addEventListener('click', async () => {
  ...
  if (!tab || !tab.url.includes('sportpesa.co.tz')) {
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  ...
  // Wallet updates: always keep stake at balance + 100 TZS.
  ...
  if (changes.lastBetResponse) {
    ...
    let nextBetAlreadyScheduled = false;
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          if (firstItem.type === 'error' || firstItem.code) {
            ...
            if (firstItem.code === 120 || (firstItem.msg && firstItem.msg.toLowerCase().includes('insufficient funds'))) {
              ...
              // Keep the active cycle timer running; the loop controls countdown timing.
            } else {
    ...
    if (hasError) {
      ...
      addBetStatusMessage('Placed bet', 'success');
      // Timer does NOT reset on successful bet - it was already stopped by GOL button
    }
    
    // If auto-sending is enabled, send next bet
    if (false && isAutoSending && !nextBetAlreadyScheduled) {
      console.log('🔁 Auto-sending enabled - sending next bet...');
      scheduleNextBet(autoCheckIntervalMs);
    }
  }
});
      ...
      func: () => {
        return new Promise((resolve, reject) => {
          try {
            ...
            xhr.open("POST", "https://sportpesa.co.tz/api/bets/history", true);
                ...
                const result = await chrome.scripting.executeScript({
                  ...
                  func: (betId) => {
                    return new Promise((resolve) => {
                      const xhr = new XMLHttpRequest();
                      xhr.open("POST", "https://sportpesa.co.tz/api/bets/cashout/options", true);
                      xhr.setRequestHeader("accept", "application/json, text/plain, */*");
                      xhr.setRequestHeader("content-type", "application/json;charset=UTF-8");
                      xhr.setRequestHeader("x-requested-with", "XMLHttpRequest");
...
// Timer variables
let autoCheckIntervalMs = 13000; // Default 13 seconds
...
document.getElementById('saveIntervalButton').addEventListener('click', () => {
  ...
  // Only restart timer if auto-sending is active
  if (isAutoSending) {
    scheduleNextBet(autoCheckIntervalMs);
  }
...
async function getPrematchTargetTab() {
  ...
  if (activeTab && activeTab.id && activeTab.url && activeTab.url.includes('sportpesa.co.tz')) {
    return activeTab;
  }
  const tabs = await chrome.tabs.query({ url: '*://sportpesa.co.tz/*' });
    ...
    const wrapped = await chrome.scripting.executeScript({
      ...
      func: async (idList) => {
        try {
          ...
          const res = await fetch('https://sportpesa.co.tz/api/bets/history', {
            method: 'POST',
            headers: {
    ...
    try {
      ...
      const wrapped = await chrome.scripting.executeScript({
        ...
        func: async (stakeValue, betsPayload) => {
          ...
          try {
            const res = await fetch('https://sportpesa.co.tz/api/bets/place', {
              method: 'POST',
              headers: {
                'accept': 'application/json, text/plain, */*',
