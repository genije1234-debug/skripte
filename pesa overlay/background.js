...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'MANUAL_COOKIE_CAPTURE') {
    ...
    chrome.scripting.executeScript({
      ...
      chrome.cookies.getAll({ url: 'https://sportpesa.co.tz' }, (chromeCookies) => {
    ...
    chrome.scripting.executeScript({
      ...
      func: (eventIds, slotNumber) => {
        ...
        eventIds.forEach((eventId, index) => {
          ...
          const url = `https://sportpesa.co.tz/api/live/events/${eventId}/details`;
  ...
  if (message.type === 'PLACE_BET') {
    ...
    chrome.scripting.executeScript({
      ...
      func: (amount, selections) => {
        ...
        // Send fetch request
        fetch('https://sportpesa.co.tz/api/live/place', {
          method: 'POST',
          headers: {
...
chrome.webRequest.onBeforeRequest.addListener(
  (details) => {
    ...
    chrome.cookies.getAll({ url: 'https://sportpesa.co.tz' }, (cookies) => {
  ...
  { urls: ['https://sportpesa.co.tz/api/users/profile*'] }
);

console.log('✅ webRequest listener registered for:', 'https://sportpesa.co.tz/api/users/profile*');
...
chrome.tabs.onUpdated.addListener(async (tabId, info, tab) => {
  ...
  if (info.status === 'loading' && (tab.url && tab.url.includes('sportpesa.co.tz'))) {
    injectedTabs.delete(tabId);
  }
  
  // Inject on complete for sportpesa
  if (info.status === 'complete' && tab.url && tab.url.includes('sportpesa.co.tz')) {
        ...
        func: () => {
          ...
          window.addEventListener('message', (event) => {
            if (event.data.type === 'SEND_CASHOUT') {
              ...
              const xhr = new XMLHttpRequest();
              xhr.open("POST", "https://sportpesa.co.tz/api/bets/cashout", true);
              xhr.setRequestHeader("accept", "application/json, text/plain, */*");
